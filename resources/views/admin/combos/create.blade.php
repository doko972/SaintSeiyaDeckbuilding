        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <h1 class="text-3xl font-bold text-white flex items-center gap-3">
                    <span class="text-4xl">⚡</span> Créer un Combo
                </h1>
            </div>
            ...
            <!-- Formulaire -->
            <form action="{{ route('admin.combos.store') }}" method="POST" class="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-8">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-4 py-3 bg-white/5 border border-white/20 rounded-xl text-white placeholder-gray-500 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition"
                              placeholder="Décrivez l'effet du combo...">{{ old('description') }}</textarea>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="card1_id" id="card1_id" required
                                ...
                                @foreach($cards as $card)
                                    <option value="{{ $card->id }}" {{ old('card1_id') == $card->id ? 'selected' : '' }}>
                                        {{ $card->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="card2_id" id="card2_id" required
                                ...
                                <option value="">Sélectionner...</option>
                                @foreach($cards as $card)
                                    <option value="{{ $card->id }}" {{ old('card2_id') == $card->id ? 'selected' : '' }}>
                                        {{ $card->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="card3_id" id="card3_id" required
                                ...
                                <option value="">Sélectionner...</option>
                                @foreach($cards as $card)
                                    <option value="{{ $card->id }}" {{ old('card3_id') == $card->id ? 'selected' : '' }}>
                                        {{ $card->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="leader_card_id" id="leader_card_id" required
                        ...
                        @foreach($cards as $card)
                            <option value="{{ $card->id }}" {{ old('leader_card_id') == $card->id ? 'selected' : '' }}>
                                {{ $card->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="attack_id" id="attack_id" required
                        ...
                        @foreach($attacks as $attack)
                            <option value="{{ $attack->id }}" {{ old('attack_id') == $attack->id ? 'selected' : '' }}>
                ...
                <div class="mb-6 p-4 bg-white/5 rounded-xl border border-white/10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="endurance_cost" id="endurance_cost" value="{{ old('endurance_cost', 0) }}" min="0"
                                   class="w-full px-4 py-3 bg-white/5 border border-white/20 rounded-xl text-white focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="cosmos_cost" id="cosmos_cost" value="{{ old('cosmos_cost', 0) }}" min="0"
                                   class="w-full px-4 py-3 bg-white/5 border border-white/20 rounded-xl text-white focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition">
                        </div>
                    </div>
                </div>
