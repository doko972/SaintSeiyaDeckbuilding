<x-app-layout>
    ...
    <div class="relative z-10 py-8 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-8">
            <a href="{{ route('admin.tournaments.show', $tournament) }}" class="text-gray-400 hover:text-white transition">
                &#8592;
            </a>
            <h1 class="text-3xl font-bold text-white flex items-center gap-3">
                <span class="text-4xl">&#127942;</span> Modifier le tournoi
            </h1>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:border-yellow-500 focus:ring-yellow-500">{{ old('description', $tournament->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="max_players" id="max_players" required
                            ...
                            <option value="8" {{ old('max_players', $tournament->max_players) == 8 ? 'selected' : '' }}>8 joueurs (3 tours)</option>
                            <option value="16" {{ old('max_players', $tournament->max_players) == 16 ? 'selected' : '' }}>16 joueurs (4 tours)</option>
                            <option value="32" {{ old('max_players', $tournament->max_players) == 32 ? 'selected' : '' }}>32 joueurs (5 tours)</option>
                        </select>
                    </div>

                    <div>
                        <label for="min_deck_cards" class="block text-sm font-medium text-gray-300 mb-2">Cartes minimum par deck *</label>
                        <input type="number" name="min_deck_cards" id="min_deck_cards" value="{{ old('min_deck_cards', $tournament->min_deck_cards) }}" min="5" max="30" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label for="entry_fee" class="block text-sm font-medium text-gray-300 mb-2">Frais d'inscription (pieces)</label>
                        <input type="number" name="entry_fee" id="entry_fee" value="{{ old('entry_fee', $tournament->entry_fee) }}" min="0" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>
            ...
            <div class="bg-gray-800/50 backdrop-blur rounded-xl border border-white/10 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" name="registration_start_at" id="registration_start_at"
                            value="{{ old('registration_start_at', $tournament->registration_start_at?->format('Y-m-d\TH:i')) }}" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label for="registration_end_at" class="block text-sm font-medium text-gray-300 mb-2">Fin des inscriptions *</label>
                        <input type="datetime-local" name="registration_end_at" id="registration_end_at"
                            value="{{ old('registration_end_at', $tournament->registration_end_at?->format('Y-m-d\TH:i')) }}" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-800/50 backdrop-blur rounded-xl border border-white/10 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="winner_coins" id="winner_coins" value="{{ old('winner_coins', $rewards['winner_coins']) }}" min="0" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label for="runner_up_coins" class="block text-sm font-medium text-gray-300 mb-2">2eme - Pieces</label>
                        <input type="number" name="runner_up_coins" id="runner_up_coins" value="{{ old('runner_up_coins', $rewards['runner_up_coins']) }}" min="0" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label for="semifinalist_coins" class="block text-sm font-medium text-gray-300 mb-2">3-4eme - Pieces</label>
                        <input type="number" name="semifinalist_coins" id="semifinalist_coins" value="{{ old('semifinalist_coins', $rewards['semifinalist_coins']) }}" min="0" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label for="participation_coins" class="block text-sm font-medium text-gray-300 mb-2">Participation - Pieces</label>
                        <input type="number" name="participation_coins" id="participation_coins" value="{{ old('participation_coins', $rewards['participation_coins']) }}" min="0" required
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-yellow-500 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label for="winner_title" class="block text-sm font-medium text-gray-300 mb-2">Titre du vainqueur</label>
                        <input type="text" name="winner_title" id="winner_title" value="{{ old('winner_title', $rewards['winner_title'] ?? '') }}" placeholder="Ex: Champion du Sanctuaire"
                            class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:border-yellow-500 focus:ring-yellow-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="exclusive_card_id" id="exclusive_card_id"
                            ...
                            @foreach($cards as $card)
                                <option value="{{ $card->id }}" {{ old('exclusive_card_id', $rewards['exclusive_card_id'] ?? '') == $card->id ? 'selected' : '' }}>
