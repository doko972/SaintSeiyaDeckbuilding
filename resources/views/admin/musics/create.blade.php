        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 overflow-hidden">
                <div class="p-6 bg-gradient-to-r from-purple-600 to-indigo-600">
                    <h3 class="text-xl font-bold text-white">🎵 Ajouter une nouvelle musique</h3>
                </div>

                <form method="POST" action="{{ route('admin.musics.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                    @csrf
                    ...
                    <!-- Fichier audio -->
                    <div>
                        <label for="file" class="block text-sm font-medium text-gray-300 mb-2">Fichier audio * (MP3, WAV, OGG - max 20MB)</label>
                        <div class="relative">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="battle" {{ old('type') == 'battle' ? 'selected' : '' }}>⚔️ Combat</option>
                                <option value="menu" {{ old('type') == 'menu' ? 'selected' : '' }}>🏠 Menu</option>
                                <option value="victory" {{ old('type') == 'victory' ? 'selected' : '' }}>🏆 Victoire</option>
                                <option value="defeat" {{ old('type') == 'defeat' ? 'selected' : '' }}>💀 Défaite</option>
                            </select>
                        </div>
                        <div>
                            <label for="volume" class="block text-sm font-medium text-gray-300 mb-2">Volume par défaut *</label>
                            <div class="flex items-center gap-4">
                                <input type="range" name="volume" id="volume" value="{{ old('volume', 50) }}" min="0" max="100"
                                    class="flex-1 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500">
                                <span id="volumeValue" class="text-white font-semibold w-12 text-center">50%</span>
                            </div>
                        </div>
                    </div>
