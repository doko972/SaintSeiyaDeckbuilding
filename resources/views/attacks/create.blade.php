            ...
            <div class="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 overflow-hidden">
                ...
                <form method="POST" action="{{ route('admin.attacks.store') }}" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-3 gap-6">
                        <div>
                            <label for="damage" class="block text-sm font-medium text-gray-300 mb-2">🗡️ Dégâts
                                *</label>
                            <input type="number" name="damage" id="damage" value="{{ old('damage', 50) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="endurance_cost" id="endurance_cost"
                                value="{{ old('endurance_cost', 20) }}" min="0" max="100" required
                        ...
                        <div>
                            ...
                            <input type="number" name="cosmos_cost" id="cosmos_cost"
                                value="{{ old('cosmos_cost', 15) }}" min="0" max="100" required
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="effect_type" id="effect_type"
                                class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-purple-500 focus:ring-purple-500">
                                <option value="none" {{ old('effect_type') == 'none' ? 'selected' : '' }}>🎯 Aucun
                                </option>
                                <option value="burn" {{ old('effect_type') == 'burn' ? 'selected' : '' }}>🔥 Brûlure
                                </option>
                                <option value="freeze" {{ old('effect_type') == 'freeze' ? 'selected' : '' }}>❄️ Gel
                                </option>
                                <option value="stun" {{ old('effect_type') == 'stun' ? 'selected' : '' }}>⚡
                                    Étourdissement</option>
                                <option value="heal" {{ old('effect_type') == 'heal' ? 'selected' : '' }}>💚 Soin
                                </option>
                                <option value="drain" {{ old('effect_type') == 'drain' ? 'selected' : '' }}>💀 Drain
                                </option>
                                <option value="buff_attack"
                                    {{ old('effect_type') == 'buff_attack' ? 'selected' : '' }}>💪 Buff Attaque
                                </option>
                                <option value="buff_defense"
                                    {{ old('effect_type') == 'buff_defense' ? 'selected' : '' }}>🛡️ Buff Défense
                                </option>
                        ...
                        <div>
                            ...
                            <input type="number" name="effect_value" id="effect_value"
                                value="{{ old('effect_value', 0) }}" min="0" max="100"
                                class="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-3 text-white focus:border-purple-500 focus:ring-purple-500">
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4 pt-4">
                        <button type="submit"
                            class="flex-1 py-3 bg-gradient-to-r from-red-600 to-orange-600 text-white font-bold rounded-lg hover:from-red-500 hover:to-orange-500 transition">
                            ✅ Créer l'attaque
                        </button>
                        <a href="{{ route('attacks.index') }}"
