<nav x-data="{ open: false, gameMenuOpen: false }" class="bg-gradient-to-r from-gray-900/95 via-purple-900/90 to-gray-900/95 backdrop-blur-md border-b border-purple-500/20 sticky top-0 z-50">
    <!-- Effet de brillance subtil -->
    <div class="absolute inset-0 bg-gradient-to-r from-transparent via-purple-500/5 to-transparent pointer-events-none"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex justify-between h-16">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-2 group">
                        ...
                        @if(file_exists($logoPath))
                            <img src="{{ asset('images/logo.webp') }}" alt="Saint Seiya Logo" class="h-10 w-auto group-hover:scale-110 transition-transform duration-300">
                        @else
                            <span class="text-2xl group-hover:scale-110 transition-transform duration-300">⚔️</span>
                        @endif
                        <span class="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-amber-300 to-yellow-400 group-hover:from-yellow-300 group-hover:to-amber-200 transition-all duration-300">
                            Saint Seiya
                        </span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-1 sm:-my-px sm:ms-10 sm:flex items-center">
                            ...
                            <div class="py-2">
                                ...
                                <a href="{{ route('game.index') }}" 
                                   ...
                                   {{ request()->routeIs('game.*') 
                                    ...
                                    <span class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-xl group-hover:scale-110 transition-transform shadow-lg">
                                        🤖
                                    </span>
                                    <div>
                                ...
                                <a href="{{ route('pvp.lobby') }}" 
                                   ...
                                   {{ request()->routeIs('pvp.*') 
                                       ...
                                       : 'text-gray-300 hover:bg-white/10 hover:text-white' }}">
                                    <span class="w-10 h-10 rounded-lg bg-gradient-to-br from-red-500 to-orange-600 flex items-center justify-center text-xl group-hover:scale-110 transition-transform shadow-lg">
                                        🆚
                                    </span>
                                    <div class="flex-1">
                    ...
                    @if(auth()->user()->isAdmin())
                        ...
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30 hover:text-red-300 transition-all duration-300">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <x-slot name="content">
                                <div class="bg-gray-900 border border-purple-500/30 rounded-lg overflow-hidden">
                                    <a href="{{ route('cards.index') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-purple-500/20 hover:text-white transition">
                                        🃏 Cartes (CRUD)
                                    </a>
                                    <a href="{{ route('attacks.index') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-purple-500/20 hover:text-white transition">
                                        ⚔️ Attaques (CRUD)
                                    </a>
                                    <a href="{{ route('admin.cards.create') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-purple-500/20 hover:text-white transition">
                                        ➕ Créer une carte
                                    </a>
                                </div>
                            </x-slot>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-3">
                <!-- Coins Display -->
                <div class="px-4 py-2 bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border border-yellow-500/30 rounded-full flex items-center gap-2 shadow-lg shadow-yellow-500/10">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-gray-300 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/30 transition-all duration-300">
                            <div class="flex items-center gap-2">
                                ...
                                @if(auth()->user()->isAdmin())
                                    <span class="px-2 py-0.5 text-xs bg-red-500/30 text-red-400 rounded border border-red-500/30">Admin</span>
                                @endif
                            </div>
                    ...
                    <x-slot name="content">
                        <div class="bg-gray-900 border border-purple-500/30 rounded-lg overflow-hidden">
                            <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-purple-500/20 hover:text-white transition">
                                👤 Profil
                            </a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-red-500/20 hover:text-red-400 transition">
                                    🚪 Déconnexion
                                </button>
                            </form>
                        </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <!-- Mobile Coins -->
                <div class="mr-2 px-3 py-1 bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border border-yellow-500/30 rounded-full flex items-center gap-1">
                    <span class="text-yellow-400 text-sm">🪙</span>
                    <span class="font-bold text-yellow-400 text-sm">{{ auth()->user()->coins ?? 0 }}</span>
                </div>
                
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 focus:outline-none transition duration-300">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-900/98 backdrop-blur-md border-t border-purple-500/20">
        <div class="pt-2 pb-3 space-y-1 px-4">
            ...
            @if(auth()->user()->isAdmin())
                <div class="pt-2 mt-2 border-t border-purple-500/20">
                    <div class="text-xs font-semibold text-red-400 uppercase tracking-wider mb-2 px-4">👑 Administration</div>
                    <a href="{{ route('cards.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-300 hover:bg-red-500/20 hover:text-white transition">
                        <span class="text-xl">🃏</span>
                        <span class="font-medium">Cartes (CRUD)</span>
                    </a>
                    <a href="{{ route('attacks.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-300 hover:bg-red-500/20 hover:text-white transition">
                        <span class="text-xl">⚔️</span>
        ...
        <div class="pt-4 pb-3 border-t border-purple-500/20 px-4">
            ...
            <div class="space-y-1">
                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-300 hover:bg-white/10 hover:text-white transition">
                    <span class="text-xl">👤</span>
                    <span class="font-medium">Profil</span>
                </a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-gray-300 hover:bg-red-500/20 hover:text-red-400 transition">
                        <span class="text-xl">🚪</span>
