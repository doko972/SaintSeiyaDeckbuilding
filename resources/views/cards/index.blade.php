    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if(auth()->user()->isAdmin())
                <a href="{{ route('admin.cards.create') }}" class="inline-flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
                    ➕ Nouvelle Carte
                </a>
            @endif
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-xl p-6 mb-8 border border-white/20">
                <form method="GET" action="{{ route('cards.index') }}" class="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <!-- Faction -->
                    <select name="faction" class="bg-white/10 border border-white/30 rounded-lg px-4 py-2 text-white focus:border-purple-500">
                        <option value="">Toutes les factions</option>
                        @foreach(\App\Models\Faction::orderBy('name')->get() as $faction)
                            <option value="{{ $faction->id }}" {{ request('faction') == $faction->id ? 'selected' : '' }}>
                                {{ $faction->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <!-- Rareté -->
                    <select name="rarity" class="bg-white/10 border border-white/30 rounded-lg px-4 py-2 text-white focus:border-purple-500">
                    ...
                    <!-- Armure -->
                    <select name="armor" class="bg-white/10 border border-white/30 rounded-lg px-4 py-2 text-white focus:border-purple-500">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('cards.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg transition">
                            ✖
                        </a>
                    </div>
                </form>
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex gap-2">
                    <span class="px-3 py-1 bg-gray-600 text-gray-300 text-sm rounded-full">{{ \App\Models\Card::where('rarity', 'common')->count() }} Communes</span>
                    <span class="px-3 py-1 bg-blue-600 text-white text-sm rounded-full">{{ \App\Models\Card::where('rarity', 'rare')->count() }} Rares</span>
                    <span class="px-3 py-1 bg-purple-600 text-white text-sm rounded-full">{{ \App\Models\Card::where('rarity', 'epic')->count() }} Épiques</span>
                    <span class="px-3 py-1 bg-gradient-to-r from-yellow-500 to-orange-500 text-white text-sm rounded-full">{{ \App\Models\Card::where('rarity', 'legendary')->count() }} Légendaires</span>
                </div>
            </div>
                        ...
                        <div class="card-grid-item group">
                            ...
                            @if(auth()->user()->isAdmin())
                                <div class="card-mini-actions">
                                    <a href="{{ route('cards.show', $card) }}" class="action-btn action-view" title="Voir">
                                        👁️
                                    </a>
                                    <a href="{{ route('admin.cards.edit', $card) }}" class="action-btn action-edit" title="Modifier">
                                        ✏️
                                    </a>
                                    <form action="{{ route('admin.cards.destroy', $card) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Supprimer {{ $card->name }} ?');">
                                        @csrf
                                        @method('DELETE')
